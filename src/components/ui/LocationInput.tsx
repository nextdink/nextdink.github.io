...
export function LocationInput({
  ...
  const getPlaceDetails = useCallback(
    async (placeId: string, name: string, address: string) => {
      ...
      try {
        ...
        if (place.location) {
          ...
          setInputValue(
            `${eventLocation.venueName} - ${eventLocation.formattedAddress}`,
          );
    ...
    return (
      <div className="space-y-1.5">
        ...
        <p className="text-xs text-amber-600">
          Google Maps API not configured. Using manual entry.
        </p>
      ...
      {isManualEntry && (
        <div className="space-y-2">
          <p className="text-xs text-slate-500">
            Enter manually, then tap "Use this address"
          </p>
          <button
            type="button"
